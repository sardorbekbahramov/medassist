...
export function BottomNav({ active, onChange, isDark }: BottomNavProps) {
  return (
    <nav
      className="fixed bottom-0 left-0 right-0 flex justify-around items-center py-2 safe-area-bottom"
      style={{
      ...
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            ...
            <span className="text-xs font-medium" style={{ opacity: isActive ? 1 : 0.6 }}>
              {tab.label}
            </span>
          </button>
        )
